...
int Analog_pin_sens = A0;         // pin voltaggio sensore
int Analog_pin_volt = A1;         // pin voltaggio tensione di riferimento
...
void loop() {
  ...
  */
  // lettura voltaggio di riferimnoto (0 to 1023)
  Vadc_read = analogRead(Analog_pin_volt);
  

  
  // calcoli senza media
  Volt_ref_calc = Vadc_read * (Volt_ref / 1024.0);
  Volt_calc = Nadc_read * (Volt_ref_calc / 1024.0);
  Temp_calc = (Volt_calc / Sens_coeff) - 273.15;
  Temp_calc_x10 = Temp_calc * 10;
  
  /*
  Volt_ref_calc = Vadc_read * (Volt_ref / 1024.0);
  Volt_calc = ((Nadc_read_sum/40.0) * (Volt_ref_calc / 1024.0));
  Temp_calc = (Volt_calc / Sens_coeff) - 273.15;
  Temp_calc_x10 = Temp_calc * 10;
  */

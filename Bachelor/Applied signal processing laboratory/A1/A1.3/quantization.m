...
support = rectangular(f,(f0-0.5)); %added arbitrarily 0.5 to avoid non-idealized filters problems
BPF = rectangular(f,(f0+0.5)) - support; %create a pass-band filter

Sq_clean =fftshift(BPF).*(Sq); %moltiplication pass-band Fourier transform with the signal to maintain desired frequencies
Noise = fftshift(BPF - 1).* (Sq); %To obtain all the other undesidered frequency and we call them noise

%% Energy evaluation 
Es = sum(abs(Sq_clean).^2)*fres;
En = sum(abs(Noise).^2)*fres; 
SNR = 10* (log10 (Es) - log10(En)); 

%% Plot frequency domain in dB
figure
hold on
grid on
box on
